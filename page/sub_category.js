...
router.post('/add',
    async(req, res) =>{
        ...
        try{
        const con = await db();
        const[result] = await con.execute("CALL sp_sub_category_add(?, ?, ?)",[
            c.module_id,
            c.category_id,
            c.sub_category_name
        ]);
...
router.post("/getall",
    async (req, res) => {
      let con = await db();
      let data = await con.execute("CALL sp_sub_category_getall()", []);
      res.json(data[0][0])
    }
  )
...
router.post("/delete/:id", [mdw.is_logged_in], async (req, res) => {
  try {
